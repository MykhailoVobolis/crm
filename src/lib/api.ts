...
// Функція для побудови URL ендпоінтів
const buildUrl = (...paths: string[]) =>
  `https://${PROJECT_TOKEN}.mockapi.io/api/v1/${paths.join('/')}`;

// Функція для перетворення об'єкта у квері параметри
const stringifyQueryParams = (params: Record<string, string>) =>
  new URLSearchParams(params).toString();

// Функція виклику fetch з отриманими параметрами. Повертає результат у форматі json з приведеними типами.
...
// Функції запитів до потрібного ендпоінта
